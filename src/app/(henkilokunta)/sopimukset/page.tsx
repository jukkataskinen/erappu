...
import { Badge, Button, EmptyState, Field, LinkButton, PageHeader, Select, Stat, Table, Td, Th } from "@/components/ui";
...
type Search = { yhtio?: string; luokka?: string; tila?: string };
...
export default async function ContractsPage({ searchParams }: { searchParams: Promise<Search> }) {
  ...
  const [companies, all] = await ctx.run((tx) =>
    Promise.all([listCompanies(tx, ctx.org.organizationId), listContracts(tx, { organizationId: ctx.org.organizationId, companyId, category })]),
  );
  ...
  return (
    <>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <Stat label="Voimassa" value={active.length} href="/sopimukset" />
        <Stat label="Päättymässä 90 päivän sisällä" value={endingSoon.length} tone={endingSoon.length ? "warn" : "ok"} href="/sopimukset?tila=paattymassa" />
        <Stat label="Vuosikustannukset yhteensä" value={formatEur(annual)} />
      </div>

      <form method="get" className="mt-6 grid gap-3 rounded-[var(--radius-panel)] border border-line bg-paper p-4 sm:grid-cols-2 lg:grid-cols-[1.4fr_1fr_1fr_auto] lg:items-end">
      ...
      <div className="mt-6">
        {visible.length === 0 ? (
          <EmptyState title="Ei sopimuksia" action={canWrite ? <LinkButton href="/sopimukset/uusi">Lisää sopimus</LinkButton> : null}>
