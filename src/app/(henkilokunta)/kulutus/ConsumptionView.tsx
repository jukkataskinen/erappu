...
import { Button, EmptyState, Field, Input, LinkButton, Notice, Panel, SectionTitle, Select, Stat, Table, Tabs, Td, Th, Textarea } from "@/components/ui";
...
export type ConsumptionSearch = { yhtio?: string; vuosi?: string; laji?: string; virhe?: string; tuotu?: string; paivitetty?: string; tallennettu?: string };

/**
 * Kulutusseuranta yhdelle yhtiölle. Globaalilla sivulla yhtiö valitaan
 * listasta, yhtiön sisällä (`fixedCompany`) valinta jätetään pois. `basePath`
 * on sivun oma osoite, jonne linkit ja lomakkeiden paluu osoittavat.
 */
export async function ConsumptionView({
  ...
  ctx: StaffContext;
  sp: ConsumptionSearch;
  fixedCompany?: { id: string; name: string; business_id: string };
  basePath: string;
  header: ReactNode;
}) {
  const companies = fixedCompany ? [fixedCompany] : await ctx.run((tx) => listCompanies(tx, ctx.org.organizationId));
  ...
  const company = fixedCompany ?? companies.find((c) => c.id === sp.yhtio) ?? companies[0];
  ...
  const link = (extra: Record<string, string | number>) => {
    const p = new URLSearchParams({ ...(fixedCompany ? {} : { yhtio: company.id }), vuosi: String(year), laji: utility });
  ...
  return (
    <>
      ...
      <Tabs items={tabs.map((u) => ({ key: u, label: UTILITY_LABEL[u], href: link({ laji: u }) }))} active={utility} />
      {heating.note ? (
