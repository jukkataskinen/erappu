...
export interface ThreadRow {
  ...
  /** Portaalin osapuoli: aloittaja tai henkilökunnan valitsema vastaanottaja (0104). */
...
const SELECT = `
  select t.id, t.organization_id, t.company_id, c.name as company_name, t.share_group_id, g.unit_label, t.created_by_user_id, t.participant_user_id,
         t.started_by_staff, coalesce(${PARTY_NAME("t.participant_user_id", "t.organization_id")}, u.full_name, u.email) as creator_name,
    ...
    left join er_users u on u.id = t.participant_user_id`;

export async function listPortalThreads(tx: Sql, userId: string): Promise<ThreadRow[]> {
  return tx.query<ThreadRow>(`${SELECT} where t.participant_user_id = $1 order by t.last_message_at desc`, [userId]);
}
...
export function mergeEntries(
  ...
  const entries: ThreadEntry[] = [
    ...
    ...attachments.map((a) => ({
      ...
      fromStaff: a.uploaded_by !== participantUserId,
...
export async function listThreadEntries(tx: Sql, threadId: string, participantUserId: string): Promise<ThreadEntry[]> {
  ...
  return mergeEntries(messages, attachments, participantUserId);
}
...
/**
 * Yhtiön portaalikäyttäjät, joille henkilökunta voi aloittaa viestin (0104).
 * Nimi osapuolirekisteristä; henkilökunta ei näe portaalikäyttäjän käyttäjäriviä.
 */
export async function listContactRecipients(tx: Sql, companyId: string): Promise<ContactRecipient[]> {
  const rows = await tx.query<{ user_id: string; name: string | null; role: ContactRecipient["role"]; share_group_id: string | null; unit_label: string | null }>(
    `select distinct a.user_id, ${PARTY_NAME("a.user_id", "a.organization_id")} as name, a.role, a.share_group_id, g.unit_label
       from er_portal_access a left join er_share_groups g on g.id = a.share_group_id
      where a.company_id = $1 and a.role in ('owner', 'resident', 'board')
        and a.starts_on <= current_date and (a.ends_on is null or a.ends_on >= current_date)`,
    [companyId],
  );
  return rows
    .map((r) => ({ userId: r.user_id, name: r.name ?? "Nimetön portaalikäyttäjä", role: r.role, shareGroupId: r.role === "board" ? null : r.share_group_id, unitLabel: r.role === "board" ? null : r.unit_label }))
    .sort((a, b) => (a.unitLabel ?? "~").localeCompare(b.unitLabel ?? "~", "fi", { numeric: true }) || a.name.localeCompare(b.name, "fi"));
}
