  ...
  return (
    <div className="mt-3 grid gap-3 rounded-xl border border-line bg-cloud/40 p-3">
      ...
      <form action="/api/kokoukset/liite" method="post" encType="multipart/form-data" className="grid gap-3 border-t border-line pt-3 sm:grid-cols-2">
        ...
        <Field label={`Lisää tiedosto (${next})`} htmlFor={`att_file_${itemId}`} hint={`PDF, kuva, XLSX tai DOCX, enintään 20 Mt. PDF:t ja PNG/JPEG-kuvat liitetään kutsun, esityslistan ja pöytäkirjan loppuun. Tallentuu yhtiön dokumentteihin, näkyy: ${visibilityLabel}.`}>
