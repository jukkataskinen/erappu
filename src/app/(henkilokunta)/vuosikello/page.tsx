...
import { Badge, Button, EmptyState, Field, LinkButton, Notice, PageHeader, Panel, SectionTitle, Select, Stat } from "@/components/ui";
...
type Search = { yhtio?: string; luokka?: string; vastuu?: string; myohassa?: string; kuitatut?: string; virhe?: string; luotu?: string };

const isUuid = (v: string | undefined) => !!v && /^[0-9a-f-]{36}$/i.test(v);

export default async function AnnualCyclePage({ searchParams }: { searchParams: Promise<Search> }) {
  ...
  const [companies, staff, tasks, done] = await ctx.run((tx) =>
    Promise.all([
      ...
      sp.kuitatut === "1" ? listRecentlyDone(tx, ctx.org.organizationId, 30) : Promise.resolve([] as TaskRow[]),
    ]),
  );
  ...
  const filterQuery = new URLSearchParams(Object.entries({ yhtio: sp.yhtio, luokka: sp.luokka, vastuu: sp.vastuu, myohassa: sp.myohassa }).filter(([, v]) => v) as [string, string][]).toString();
  const back = `/vuosikello${filterQuery ? `?${filterQuery}` : ""}`;
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <>
            ...
            <LinkButton href={`/vuosikello/uusi${companyId ? `?yhtio=${companyId}` : ""}`}>Uusi tehtävä</LinkButton>
          </>
        }
      />
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <Stat label="Myöhässä" value={overdue.length} tone={overdue.length ? "alert" : "ok"} href="/vuosikello?myohassa=1" />
        <Stat label="Tällä viikolla" value={thisWeek} />
        <Stat label="Seuraavat 30 päivää" value={next30} />
      </div>

      <form method="get" className="mt-6 grid gap-3 rounded-[var(--radius-panel)] border border-line bg-paper p-4 sm:grid-cols-2 lg:grid-cols-[1.4fr_1fr_1fr_auto_auto] lg:items-end">
        ...
        <div className="flex gap-2">
          ...
          {filterQuery ? <LinkButton variant="ghost" href="/vuosikello">Tyhjennä</LinkButton> : null}
        </div>
      </form>
        ...
        <div className="grid content-start gap-6">
          {overdue.length > 0 ? (
            <Panel className="border-coral/40">
              ...
              <TaskList tasks={overdue} today={today} back={back} />
            </Panel>
          ) : null}

          {!overdueOnly && tasks.length === 0 ? (
            <EmptyState title="Ei tehtäviä seuraavalle 12 kuukaudelle" action={companies[0] ? null : <LinkButton href="/taloyhtiot">Lisää taloyhtiö</LinkButton>}>
                  ...
                  <section key={m} aria-labelledby={`kk-${m}`}>
                    ...
                    {items.length ? (
                      <div className="rounded-[var(--radius-panel)] border border-line bg-paper px-4">
                        <TaskList tasks={items} today={today} back={back} />
                      </div>
                    ) : (
        ...
        <aside className="grid content-start gap-6">
          <Panel>
            ...
            <form action={createAnnualCycleAction} className="grid gap-3">
              ...
              <div>
                <Button variant="secondary" disabled={companies.length === 0}>Luo vuosikello</Button>
              </div>
            </form>
          </Panel>
        </aside>
...
function TaskList({ tasks, today, back }: { tasks: TaskRow[]; today: string; back: string }) {
  return (
    <ul className="divide-y divide-line">
      {tasks.map((t) => {
