...
import { canSimulateSigning, EsinettiError, getEsinettiClient, type EsinettiClient, type Round, type WebhookEvent } from "@/lib/esinetti";
import { completeMockRound } from "@/lib/esinetti/mock";
import { readStoredFile } from "@/lib/storage";
import { processSigningEvent } from "./signing";
 ...
 * ESINETTI_MODE=http tai NODE_ENV=production.
 *
...
export async function simulateSigning(db: Database, signingRoundId: string, userSub: string): Promise<void> {
  ...
  const [row] = await db.asUser(userSub, (tx) =>
    tx.query<{ esinetti_round_id: string; subject_id: string; storage_path: string | null; signers: { name: string; email: string; role: string }[] }>(
      `select r.esinetti_round_id, r.subject_id, d.storage_path, r.signers
  ...
  try {
    ...
    round = {
      id: row.esinetti_round_id, title: "", status: "completed", sequential: false, externalRef: `erappu:meeting:${row.subject_id}`,
      expiresAt: null, completedAt: now, createdAt: now,
      documents: [{ id: docId, name: "poytakirja.pdf", position: 0, pageCount: null, sizeBytes: original.length, originalSha256: null, sealedSha256: null }],
